...
class JavaGeneratorBase extends GeneratorBase {
    String _parentName
    JavaGeneratorConfiguration _configuration
    ...
    def generateOperationSignature(Interface type, Operation op) {
        '''«generateReturnType(type, op)» «op.name.toCamelCase»(«generateParameters(type, op)»)'''
    }
